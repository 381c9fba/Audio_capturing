...
def train_main_tag_classifier(X_train, y_train, X_val, y_val):
    """
    Функция для обучения классификатора для основных тегов с использованием CatBoost.
    ...
    pipeline = Pipeline(
        [
            ...
            ('clf', OneVsRestClassifier(CatBoostClassifier(iterations=500, learning_rate=0.1, depth=6, verbose=False)))
        ]
    )
...
def train_sub_tag_classifier(main_tag_preds, y_sub_tags):
    """
    Функция для обучения классификатора для под-тегов с использованием CatBoost.
...
def main():
    ...
    (X_train, y_main_train), (X_val, y_main_val), (X_test, y_main_test) = train_test_split(df['полный_текст'], df_main_tags_binarized, test_size=0.2, random_state=42)
    ...
    joblib.dump({'main_tag_classifier': main_tag_classifier, 'sub_tag_classifier': sub_tag_classifier, 'mlb_main': mlb_main, 'mlb_sub': mlb_sub}, 'tag_classifiers_catboost.joblib')
